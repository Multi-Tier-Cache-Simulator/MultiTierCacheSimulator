    ...
    def read_data_line(self, env, name_lock, res, forwarder, line, log_file, logs_enabled=True):
        ...
        with name_lock.request() as lock:
            ...
            if in_index:
                ...
                if ti.name.__str__() != forwarder.get_default_tier().name:
                    if priority == 'h':
                ...
                yield env.process(ti.read_packet(env, res, packet))
        ...
        with name_lock.request() as lock:
            ...
            if in_index:
                ...
                yield env.process(tier.write_packet(env, res, packet))

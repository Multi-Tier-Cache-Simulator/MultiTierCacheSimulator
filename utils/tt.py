...
num_requests = 10000
# Define the items to be requested
items = [1, 2, 3, 4, 5]
...
alpha_pareto = 0.7
xm = num_items*0.5
# Create a list to store the recently accessed items
recently_accessed = []
# Generate requests using a combination of Zipfian and Pareto distributions
    ...
    for i in range(num_requests):
        ...
        if rand <= 0.5:
            if recently_accessed:
                # Generate a request using the Zipfian distribution on recent requests
                item = recently_accessed[np.random.randint(len(recently_accessed))]
            else:
                # Generate a request using the Zipfian distribution
                rank = np.random.zipf(alpha, 1)
                while rank >= len(items):
                    rank = np.random.zipf(alpha, 1)
                item = items[rank - 1]
        else:
            # Generate a request using the Zipfian distribution
            rank = np.random.pareto(alpha_pareto, 1)+xm
            while rank >= len(items):
                rank = np.random.pareto(alpha_pareto, 1)+xm
            item = items[int(rank)]
        # Add the recently accessed item to the list
        recently_accessed.append(item)
        if len(recently_accessed) > 2:
            recently_accessed.pop(0)
        # write the request to the trace file
        trace_file.write("{},{},{},{},{},{},{}\n".format("d", req_time, item, size, priority, lifetime, response_time))

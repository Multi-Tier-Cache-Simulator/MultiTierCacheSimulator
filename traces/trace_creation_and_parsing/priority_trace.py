...
class PriorityTrace(Trace):
    ...
    def read_data_line(self, env, res, forwarder, line, log_file, logs_enabled=True):
        ...
        if in_index:
            ...
            tier.read_packet(env, res, packet)
            tier.chr += 1
            if priority == 'l':
                tier.chr_lpc += 1
            else:
        ...
        if in_index:
            ...
            if packet.priority == "h":
                ...
                forwarder.get_default_tier().write_packet(env, res, packet)
            else:
                print("%s low priority, write to lower-tier" % packet.name)
                forwarder.get_last_tier().write_packet(env, res, packet)

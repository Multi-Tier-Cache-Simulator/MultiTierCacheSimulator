...
# size is in byte
# if prefetching or high write in MRU of t2
# if the data popping from t2 is low or high with little penalty send to disk

class LatestPolicy(Policy):
    def __init__(self, env: Environment, forwarder: Forwarder, tier: Tier):
        ...
        self.nb_capacity_all = math.trunc(
            self.tier.max_size * self.tier.target_occupation / forwarder.slot_size) + math.trunc(
            self.tier.max_size * self.forwarder.tiers[
                self.forwarder.tiers.index(self.tier) + 1].target_occupation / forwarder.slot_size)
    ...
    def on_packet_access(self, env: Environment, res, packet: Packet, is_write: bool, cause=None):
        ...
        if not is_write and self.t1.__contains__(packet.name):
            ...
            if packet.priority == "h":
                ...
                self.t1.remove(packet.name)
                self.t2.append_by_index(round(self.nb_packets_capacity / get_alpha()), packet.name, packet)
        ...
        if not is_write and self.t2.__contains__(packet.name):
            ...
            if packet.priority == "h":
                ...
                self.t2.__str__()
                index = self.t2.__index__(packet.name)
                self.t2.remove(packet.name)
                self.t2.append_by_index(index + round(self.nb_packets_capacity / get_alpha()), packet.name, packet)
            with res[0].request() as req:
                yield req
                print('%s starting at %s' % (self.tier.name, env.now))
                # read
        ...
        if packet.priority == "h":
            print(packet.name + " high priority packet, insert in MRU in T1")
            self.t1.append_left(packet.name, packet)
        else:
            print(packet.name + " low priority packet, insert in t1")
            self.t1.append_by_index(round(self.nb_packets_capacity / get_alpha()), packet.name, packet)
        # increment number of writes
        self.tier.used_size += packet.size
        self.tier.number_of_packets += 1
        self.tier.number_of_write += 1
